...
var door = load("res://Maze/Door.tscn") # place door on random tile


var tile = 6  # tile size (in meters)
...
func make_maze():
	...
	var index_x = rand_range(0, map.size())
	var index_y = rand_range(0, map[0].size())
	var doorPlaced = false
	for x in range(width):
		for y in range(height):
			var t = tiles[map[x][y]].instance()
			if randf() < 0.05 and not doorPlaced:
				t.add_child(door.instance())
				doorPlaced = true
			if not doorPlaced and x == 17 and y == 5:
				t.add_child(door.instance())
				doorPlaced = true
			t.translation = Vector3(x, 0, y)*tile
